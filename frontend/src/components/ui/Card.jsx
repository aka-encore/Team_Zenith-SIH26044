...
export function CardHeader({ title, subtitle, action, className = '' }) {
  return (
    <div className={`flex items-start justify-between pb-4 border-b border-slate-200 dark:border-slate-800 mb-4 ${className}`}>
      <div>
...
export function CardFooter({ children, className = '' }) {
  return (
    <div className={`pt-4 border-t border-slate-200 dark:border-slate-800 mt-4 text-xs text-slate-500 dark:text-slate-400 flex items-center justify-between ${className}`}>
      {children}
    </div>
  );
}
  ...
  return (
    <Card className={`space-y-2 ${className}`}>
      ...
      {subtitle ? (
        ...
        <p className="text-[11px] text-slate-400 dark:text-slate-500 italic">No data recorded yet</p>
      ) : null}
      {trend && (
        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md inline-block font-mono ${
          trend.isPositive ? 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-500/20' : 'bg-rose-500/10 text-rose-700 dark:text-rose-400 border border-rose-500/20'
        }`}>
          {trend.value}
        </span>
      )}
