...
import {
  ...
  FileSearch, Lightbulb, UserCheck, Network, Dna
} from 'lucide-react';
    ...
    <div className="space-y-20 sm:space-y-28 pb-16">
      ...
      <section className="relative pt-6 pb-12 overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto text-center space-y-8 px-4">
          ...
          {/* Top badge */}
          <div className="inline-flex items-center space-x-2 px-4 py-2 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-semibold">
            <Sparkles className="h-3.5 w-3.5" />
            <span>Smart India Hackathon 2026 — SIH26044 — Team Zenith</span>
          </div>
          ...
          <div className="space-y-4">
            ...
            <p className="text-slate-400 text-base sm:text-lg max-w-2xl mx-auto leading-relaxed">
              <strong className="text-white">SkillNexus AI</strong> — a competency intelligence platform connecting AYUSH students, academic institutions, and healthcare industries through AI-powered skill mapping and opportunity matching.
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-2">
            <button
              ...
              className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold px-8 py-3.5 rounded-xl text-sm transition shadow-lg shadow-emerald-600/25 flex items-center space-x-2 cursor-pointer"
            >
            ...
            <button
              ...
              className="bg-slate-800 hover:bg-slate-700 text-white font-bold px-8 py-3.5 rounded-xl text-sm transition border border-slate-700 flex items-center space-x-2 cursor-pointer"
            >
          ...
          {/* Skill radar preview card */}
          <div className="mt-12 max-w-lg mx-auto">
            <div className="glass-card p-5 rounded-2xl border border-slate-800 text-left space-y-4">
              <div className="flex items-center justify-between text-xs text-slate-400 pb-3 border-b border-slate-800">
                <span className="font-semibold text-white flex items-center space-x-2">
                  ...
                  <span>SkillNexus AI — Competency Radar</span>
                </span>
                <span className="text-emerald-400 font-semibold">Riya Sharma • BAMS 4th Yr</span>
              </div>
      ...
      {/* ═══════════════════════════════════════════════════════════
          SECTION 4: ROLE-BASED TABS
          ═══════════════════════════════════════════════════════════ */}
      <section className="max-w-7xl mx-auto px-4">
        <div className="text-center max-w-3xl mx-auto mb-10">
          <h2 className="text-xs uppercase tracking-widest text-emerald-400 font-bold mb-2">Tailored for Everyone</h2>
          <h3 className="text-3xl sm:text-4xl font-bold text-white">
        ...
        <div className="flex justify-center mb-8">
          <div className="inline-flex bg-slate-900 border border-slate-800 rounded-2xl p-1 gap-1">
            {Object.entries(roleTabData).map(([key, role]) => {
              const RoleIcon = role.icon;
              const isActive = activeRoleTab === key;
              const accentActive = {
                student: 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/20',
                industry: 'bg-blue-600 text-white shadow-lg shadow-blue-600/20',
                faculty: 'bg-amber-600 text-white shadow-lg shadow-amber-600/20'
              };
              return (
                <button
                  key={key}
                  ...
                  className={`flex items-center space-x-2 px-4 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${
                    isActive ? accentActive[key] : 'text-slate-400 hover:text-white'
                  }`}
                >
                  <RoleIcon className="h-3.5 w-3.5" />
                  <span className="capitalize">{key === 'faculty' ? 'Faculty' : key === 'student' ? 'Students' : 'Industry'}</span>
                </button>
              );
            })}
          </div>
        ...
        {(() => {
          ...
          const RoleIcon = role.icon;
          const accentColors = {
            student: { text: 'text-emerald-400', bg: 'bg-emerald-500/10', border: 'border-emerald-500/20', btn: 'bg-emerald-600 hover:bg-emerald-500' },
            industry: { text: 'text-blue-400', bg: 'bg-blue-500/10', border: 'border-blue-500/20', btn: 'bg-blue-600 hover:bg-blue-500' },
            faculty: { text: 'text-amber-400', bg: 'bg-amber-500/10', border: 'border-amber-500/20', btn: 'bg-amber-600 hover:bg-amber-500' }
          };
          const ac = accentColors[activeRoleTab];

          return (
            <div className="glass-card rounded-2xl border border-slate-800 p-6 sm:p-10 grid grid-cols-1 lg:grid-cols-2 gap-10">

              {/* Left — Role description */}
              <div className="space-y-6 flex flex-col justify-between">
                <div className="space-y-4">
                  <div className={`inline-flex items-center space-x-2 px-3 py-1.5 rounded-lg ${ac.bg} border ${ac.border} ${ac.text} text-xs font-bold`}>
                ...
                <button
                  ...
                  className={`${ac.btn} text-white font-semibold px-6 py-3 rounded-xl text-sm transition-all shadow-md flex items-center space-x-2 cursor-pointer w-fit`}
                >
        ...
        <div className="glass-card p-10 sm:p-16 rounded-3xl border border-emerald-500/20 text-center relative overflow-hidden bg-gradient-to-r from-emerald-950/40 via-slate-900/80 to-teal-950/40">
          ...
          <div className="max-w-3xl mx-auto space-y-6 relative z-10">
            ...
            <p className="text-slate-300 text-base sm:text-lg leading-relaxed">
              Join 1,200+ students, 50+ industry partners, and 20+ academic institutions building the future of competency-driven AYUSH healthcare workforce development with <strong>SkillNexus AI</strong>.
            </p>
      ...
      {/* ═══════════════════════════════════════════════════════════
          SECTION 7: FOOTER (inline — rich navigation)
          ═══════════════════════════════════════════════════════════ */}
      <footer className="max-w-7xl mx-auto px-4 pt-12 border-t border-slate-800">
        ...
        <div className="border-t border-slate-800 py-6 flex flex-col sm:flex-row items-center justify-between gap-3 text-[11px] text-slate-500">
          <span>© 2026 SkillNexus AI — AYUSH Academia-Industry Collaboration Portal — Team Zenith. Built for Smart India Hackathon 2026 (SIH26044).</span>
