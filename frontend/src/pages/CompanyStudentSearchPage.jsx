...
import { 
  ...
  ExternalLink, Layers, ShieldCheck, Briefcase, Plus
} from 'lucide-react';
...
export default function CompanyStudentSearchPage() {
  ...
  // Fetch students from backend with active query params
  ...
  return (
    <div className="space-y-8 pb-20 text-left max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Discover verified student talent across universities by technical skills, department, CGPA, and Skill DNA proficiency levels.
          </p>
        </div>
      ...
      <div className="glass-card p-5 sm:p-6 rounded-3xl border border-slate-200 dark:border-slate-800 bg-white/70 dark:bg-slate-900/60 shadow-sm space-y-4">
        ...
        <form onSubmit={handleApplyFilters} className="flex flex-col md:flex-row items-center gap-3">
          <div className="relative flex-1 w-full">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-slate-100 outline-none focus:border-emerald-500"
            />
          </div>

          <div className="relative flex-1 w-full">
            <Sparkles className="h-4 w-4 absolute left-3.5 top-3.5 text-emerald-500" />
            <input
              type="text"
              placeholder="Filter by specific skill (e.g. React, Python, Docker)..."
              value={skillFilter}
          ...
          <button
            type="submit"
            className="w-full md:w-auto px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-extrabold transition shadow-md shadow-emerald-600/20 cursor-pointer flex items-center justify-center space-x-2 shrink-0"
          >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2 border-t border-slate-200/80 dark:border-slate-800/80">
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-300 outline-none focus:border-emerald-500 cursor-pointer"
            >
        ...
        {(searchQuery || skillFilter || deptFilter !== 'all' || yearFilter !== 'all' || minCgpaFilter !== 'all' || skillLevelFilter !== 'all') && (
          <div className="flex items-center justify-between pt-2 text-xs">
            <span className="text-slate-400 font-medium">Showing filtered candidate results ({students.length})</span>
            <button
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center p-16 text-slate-500 space-y-3">
          ...
          <span className="text-xs font-mono font-bold uppercase tracking-wider">Scanning Verified Student Talent...</span>
        </div>
      ) : students.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {students.map(student => (
            <div 
              key={student._id}
              className="glass-card p-6 rounded-3xl border border-slate-200 dark:border-slate-800 bg-white/70 dark:bg-slate-900/70 hover:border-emerald-400/40 dark:hover:border-emerald-500/30 transition shadow-sm flex flex-col justify-between space-y-4 text-left"
            >
              <div className="space-y-3.5">
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-center space-x-3">
                    <div className="w-11 h-11 rounded-2xl bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 font-extrabold flex items-center justify-center text-base shrink-0 overflow-hidden">
                      {student.avatarUrl ? (
                ...
                <div className="space-y-1.5 pt-1 border-t border-slate-100 dark:border-slate-800">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {student.skills.slice(0, 5).map((sk, skIdx) => (
                      ...
                      >
                        <span>{sk.name}</span>
                        <span className="text-[8px] text-emerald-500 uppercase font-extrabold">({sk.proficiencyLevel.slice(0, 3)})</span>
                      </span>
                    ))}
              ...
              <div className="pt-3 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between gap-2">
                ...
                <button
                  ...
                  className="flex-1 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-extrabold transition flex items-center justify-center space-x-1.5 cursor-pointer shadow-md shadow-emerald-600/20 active:scale-95"
                >
          ...
          <div className="glass-card max-w-2xl w-full p-6 sm:p-8 rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 space-y-6 shadow-2xl max-h-[90vh] overflow-y-auto">
                  ...
                  {selectedStudent.projects.map((proj, pIdx) => (
                    <div key={pIdx} className="p-3 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 text-xs space-y-1">
                      <div className="flex items-center justify-between">
                        ...
                        {proj.link && (
                          <a href={proj.link} target="_blank" rel="noreferrer" className="text-emerald-500 hover:underline flex items-center space-x-1 text-[11px]">
            ...
            {selectedStudent.resumeUrl && (
              <div className="p-3.5 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 flex items-center justify-between text-xs">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex items-center space-x-1.5 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg font-bold"
                >
            ...
            <div className="pt-4 border-t border-slate-200 dark:border-slate-800 flex items-center justify-end space-x-3 text-xs">
              ...
              <button
                ...
                className="px-5 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-extrabold transition shadow-md shadow-emerald-600/20 cursor-pointer flex items-center space-x-1.5"
              >
      ...
      {shortlistModalStudent && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="glass-card max-w-md w-full p-6 sm:p-8 rounded-3xl border border-emerald-500/30 bg-white dark:bg-slate-950 space-y-5 shadow-2xl">
            ...
            <form onSubmit={handleConfirmShortlist} className="space-y-4 text-xs">
              ...
              <div className="flex items-center justify-end space-x-3 pt-3 border-t border-slate-200 dark:border-slate-800">
                ...
                <button
                  ...
                  className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-extrabold shadow-md shadow-emerald-600/20 disabled:opacity-50 transition cursor-pointer flex items-center space-x-2"
                >
                  {shortlisting ? (
                    <>
